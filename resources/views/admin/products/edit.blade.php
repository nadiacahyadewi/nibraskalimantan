...
<div class="max-w-4xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-2">
            ...
            <div>
                ...
                <p class="text-gray-500 text-sm mt-1">Perbarui informasi produk: <span class="font-medium text-nibras-magenta">{{ $product->name }}</span></p>
            </div>
        </div>
    ...
    <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data" id="product-form" class="space-y-6 pb-12">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div class="p-6 space-y-4">
                <div>
                    ...
                    <input type="text" name="name" id="name" required value="{{ old('name', $product->name) }}" class="w-full px-4 py-2 rounded-lg border border-gray-200 focus:border-nibras-magenta focus:ring-1 focus:ring-nibras-magenta outline-none transition-all">
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="price" id="price" value="{{ old('price', (int)$product->price) }}" readonly class="w-full pl-12 pr-4 py-2 rounded-lg border border-gray-200 bg-gray-50 text-gray-500 cursor-not-allowed outline-none currency-input" title="Harga dasar otomatis disinkronisasi dari varian Anda">
                        </div>
                    </div>
                    <div>
                        <label for="color" class="block text-sm font-medium text-gray-700 mb-1">Warna</label>
                        <input type="text" name="color" id="color" value="{{ old('color', $product->color) }}" class="w-full px-4 py-2 rounded-lg border border-gray-200 focus:border-nibras-magenta focus:ring-1 focus:ring-nibras-magenta outline-none transition-all">
                    </div>
                    <div>
                        <label for="weight" class="block text-sm font-medium text-gray-700 mb-1">Berat Produk (Gram)</label>
                        <div class="relative">
                            <input type="number" name="weight" id="weight" value="{{ old('weight', $product->weight) }}" placeholder="500" class="w-full px-4 py-2 rounded-lg border border-gray-200 focus:border-nibras-magenta focus:ring-1 focus:ring-nibras-magenta outline-none transition-all pr-12">
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="4" class="w-full px-4 py-2 rounded-lg border border-gray-200 focus:border-nibras-magenta focus:ring-1 focus:ring-nibras-magenta outline-none transition-all">{{ old('description', $product->description) }}</textarea>
                </div>
            </div>
        </div>
                    ...
                    <tbody id="variants-container" class="divide-y divide-gray-100 block md:table-row-group">
                        ...
                        <tr class="variant-row group hover:bg-gray-50/50 transition-colors block md:table-row border-b md:border-b-0 border-gray-100 p-4 md:p-0">
                            ...
                            <td class="px-0 md:px-6 py-2 md:py-3 block md:table-cell">
                                ...
                                <input type="text" name="variants[{{ $index }}][size]" value="{{ old('variants.'.$index.'.size', $variant->size) }}" required class="w-full px-3 py-1.5 rounded-md border border-gray-200 focus:border-nibras-magenta outline-none text-sm">
                            </td>
                             <td class="px-0 md:px-6 py-2 md:py-3 block md:table-cell">
                                <label class="block md:hidden text-xs font-semibold text-gray-500 mb-1">Harga Jual (Rp)</label>
                                <input type="text" name="variants[{{ $index }}][price]" value="{{ old('variants.'.$index.'.price', (int)$variant->price) }}" required class="w-full px-3 py-1.5 rounded-md border border-gray-200 focus:border-nibras-magenta outline-none text-sm variant-price-input currency-input">
                            </td>
                            <td class="px-0 md:px-6 py-2 md:py-3 block md:table-cell">
                                <label class="block md:hidden text-xs font-semibold text-gray-500 mb-1">Harga Diskon (Rp)</label>
                                <input type="text" name="variants[{{ $index }}][discount_price]" value="{{ old('variants.'.$index.'.discount_price', $variant->discount_price ? (int)$variant->discount_price : '') }}" placeholder="Opsional" class="w-full px-3 py-1.5 rounded-md border border-gray-200 focus:border-nibras-magenta outline-none text-sm variant-discount-input currency-input">
                            </td>
                            <td class="px-0 md:px-6 py-2 md:py-3 block md:table-cell">
                                <label class="block md:hidden text-xs font-semibold text-gray-500 mb-1">Harga Modal (Rp)</label>
                                <input type="text" name="variants[{{ $index }}][purchase_price]" value="{{ old('variants.'.$index.'.purchase_price', (int)$variant->purchase_price) }}" required class="w-full px-3 py-1.5 rounded-md border border-gray-200 focus:border-nibras-magenta outline-none text-sm currency-input">
                            </td>
                            <td class="px-0 md:px-6 py-2 md:py-3 block md:table-cell">
                                <label class="block md:hidden text-xs font-semibold text-gray-500 mb-1">Stok</label>
                                <input type="number" name="variants[{{ $index }}][stock]" value="{{ old('variants.'.$index.'.stock', $variant->stock) }}" required min="0" class="w-full md:w-32 px-4 py-2 rounded-md border-2 border-gray-200 focus:border-nibras-magenta outline-none text-base font-bold text-center mx-auto">
                            </td>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function syncBasePrice() {
            ...
            document.querySelectorAll('.variant-row').forEach(row => {
                ...
                if (priceInput && (priceInput.value || (discountInput && discountInput.value))) {
